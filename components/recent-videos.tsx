  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-display text-4xl leading-none">Latest Broadcast</h2>
        <Link href={siteConfig.routes.video} className="text-sm text-[#C45A3C] hover:underline">
          Open Broadcast
        </Link>
      </div>

      {videos.length === 0 ? (
        <p className="italic text-muted-foreground">No broadcast available yet.</p>
      ) : (
            ...
            <Card key={video.id} className="overflow-hidden border-[#E8E5E0]/15 bg-[#111111]/70">
              ...
              <CardHeader className="pb-2">
                <CardTitle className="text-lg">
                  <Link href={video.url} target="_blank" rel="noopener noreferrer" className="hover:underline">
                    {video.title}
                  </Link>
                </CardTitle>
              ...
              <CardContent>
                <p className="text-sm text-[#d2ccc3] line-clamp-2">{video.description}</p>
              </CardContent>
              <CardFooter>
