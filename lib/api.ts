...
export async function fetchTweets(): Promise<Tweet[]> {
  try {
    ...
    const tweetsResponse = await fetch(
      `https://api.twitter.com/2/users/${userId}/tweets?max_results=50&tweet.fields=created_at,public_metrics,attachments&expansions=attachments.media_keys&media.fields=url`,
      {
        headers: {
          Authorization: `Bearer ${BEARER_TOKEN}`,
        },
    ...
    const tweets: Tweet[] =
      tweetsData.data?.map((tweet: any) => ({
        id: tweet.id,
        content: tweet.text,
        date: formatRelativeTime(new Date(tweet.created_at)),
        ...
        author: {
          ...
          avatar: userData.data.profile_image_url || "/placeholder.svg?height=40&width=40",
        },
        hasImage: tweet.attachments?.media_keys?.length > 0,
        imageUrl:
          tweet.attachments?.media_keys?.length > 0
            ? tweetsData.includes?.media?.find((media: any) => tweet.attachments.media_keys.includes(media.media_key))
                ?.url
            : undefined,
